...
class BotState:
    ...
    def record_trade(self, result: dict):
        ...
        if result.get('success'):
            ...
            # Only count loss if we had PARTIAL fills (real exposure)
            # Fully rejected orders cost nothing
            n_filled = result.get('n_filled', 0)
            if n_filled > 0:
                loss = result.get('total_cost', 0) * 0.1
                self.total_loss += loss
                self.current_bankroll -= loss
            # else: all rejected, no money spent, no loss
...
def scan_and_execute(client: PolyClient, state: BotState):
    ...
    for opp in opportunities:
        ...
        n_outcomes = len(market.get('yes_prices', []))
        per_leg = position_size / max(n_outcomes, 1)
        if per_leg < 1.0:
            logger.warning(f"  ⛔ Skipping {market['question'][:40]}: "
                          f"${per_leg:.2f}/leg < $1 min ({n_outcomes} outcomes, ${position_size:.2f} total)")
            continue
