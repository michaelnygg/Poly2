...
def scan_and_execute(client: PolyClient, state: BotState):
    ...
    for event in events_raw:
        ...
        # Collect YES price for each market in the event
        yes_prices = []
        token_ids = []
        outcomes = []
        valid = True
        ...
        parsed_market = {
            ...
            'token_ids_yes': token_ids,
            'token_ids_no': [''] * len(yes_prices),
            'volume': 0,
    ...
    for opp in opportunities:
        ...
        elif atype == 'sell_all_yes':
            result = client.execute_sell_all_yes(market, position_size, dry_run=is_dry)
        else:
            continue
...
def main():
    ...
    if DRAWDOWN_LIMIT > 1.0:
        logger.error(f"⚠️  DRAWDOWN_LIMIT={DRAWDOWN_LIMIT} looks wrong! Should be 0.15 (15%), not 15.")
        logger.error("   Auto-correcting: using 0.15")
    if MAX_POSITION_PCT > 1.0:
        logger.error(f"⚠️  MAX_POS_PCT={MAX_POSITION_PCT} looks wrong! Should be 0.10 (10%), not 10.")
        logger.error("   Auto-correcting: using 0.10")
    if MIN_PROFIT > 1.0:
        logger.error(f"⚠️  MIN_PROFIT={MIN_PROFIT} looks wrong! Should be 0.01-0.05, not {MIN_PROFIT}.")
        logger.error("   Auto-correcting: dividing by 100")
    
    # Override globals with corrected values
    global DRAWDOWN_LIMIT, MAX_POSITION_PCT, MIN_PROFIT
    DRAWDOWN_LIMIT = DRAWDOWN_LIMIT if DRAWDOWN_LIMIT <= 1.0 else DRAWDOWN_LIMIT / 100.0
    MAX_POSITION_PCT = MAX_POSITION_PCT if MAX_POSITION_PCT <= 1.0 else MAX_POSITION_PCT / 100.0
    MIN_PROFIT = MIN_PROFIT if MIN_PROFIT <= 1.0 else MIN_PROFIT / 100.0
